...
dist_df         <- fread(paste0(rslt_dir, treatment, "/smccnet_isns_euclidean_dist_mtrx.csv"))
dist_mtrx       <- as.dist(dist_df)
...
dist_df      <- mutate(dist_df, DNA_ID = colnames(dist_df), cluster = as.factor(cut_avg))
count(dist_df,cluster)
...
pca_mtrx <- t(cbind(dnam_mtrx, snps_mtrx))

pca_res <- prcomp(pca_mtrx, scale = T, center = T)
pca_eigenvec <- as.data.frame(pca_res$rotation)
...
plt.size <- 10
ggplot(plt_df, aes(x = cluster, y = avg_dnam_beta)) +
