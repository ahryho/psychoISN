...
dex_net_cpg_loc <- cpg_loc[CpG_ID %in% dex_global_net_flatten_sign$row] %>% 
  mutate(ID = CpG_ID, Type = "CpG", treatment = "GR-stimulated") %>% dplyr::select(-c(CpG_ID))
dex_net_snp_loc <- snp_loc[SNP %in% dex_global_net_flatten_sign$column] %>% 
  mutate(ID = SNP, Type = "SNP", treatment = "GR-stimulated") %>% dplyr::select(-SNP)

veh_net_cpg_loc <- cpg_loc[CpG_ID %in% veh_global_net_flatten_sign$row] %>% 
  mutate(ID = CpG_ID, Type = "CpG", treatment = "Baseline") %>% dplyr::select(-c(CpG_ID))
veh_net_snp_loc <- snp_loc[SNP %in% veh_global_net_flatten_sign$column] %>% 
...
veh_qtl_net <- veh_global_net_flatten_sign[row %like% "cg"][column %like% "rs"] %>%
  dplyr::select(SNP = row, CpG_ID = column, value = cor, fdr)
...
# Take only QTLs
dex_qtl_net <- dex_global_net_flatten_sign[row %like% "cg"][column %like% "rs"] %>%
  dplyr::select(SNP = row, CpG_ID = column, value = cor, fdr)
...
```{r}
CustomChordDiagram(veh_global_net$network, cpg_loc, snp_loc, thrsh = 0.40)
```
...
```{r}
CustomChordDiagram(dex_global_net$network, cpg_loc, snp_loc, thrsh = 0.40)
```
