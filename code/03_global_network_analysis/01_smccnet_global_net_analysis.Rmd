...
```{r}
dex_features <- rownames(dex_global_net)
length(dex_features)

veh_features <- rownames(veh_global_net)
length(veh_features)

olap <- intersect(dex_features, veh_features) # 
length(olap)

olap_cpgs <- olap[grepl("cg", olap, ignore.case = T)] # 
olap_snps <- olap[grepl("rs", olap, ignore.case = T)] # 
```
...
plt <- plot(
  euler.tbl, quantities = list(type = c("counts", "percent")),
  fills = alpha(cbPalette[c(2, 1)], 0.5),
  col = alpha(cbPalette[c(2, 1)], 1),
  labels =  c("GR-stimulated", "Baseline"))

ggsave(
  paste0(plt_dir, "f01_gl_net_cpg_eulerr.pdf"),
  plt,
  width = standard_width_mm_double,
  height = standard_height_mm, 
  units = "mm",
  dpi = standard_dpi_fig, 
  scale = 1)
...
ggsave(
  paste0(plt_dir, "f02_gl_net_snps_eulerr.pdf"),
  plt,
  width = standard_width_mm_double,
  height = standard_height_mm, 
  units = "mm",
...
```{r}
plt <- ggplot(net_loc_count, aes(x = treatment, y = freq, fill = Type, color = Type)) + 
  ...
  labs(title = "", x = "", y = "Variant frequency") +
  theme(legend.title = element_blank(), 
        legend.position = "bottom",
        panel.grid.major = element_blank(),
        panel.background = element_blank(),
        plot.title = element_text(size = standard_textsize),
        axis.title = element_text(size = standard_textsize, colour = "black"),
...
ggsave(
  paste0(plt_dir, "f03_gl_net_feature_distribution.pdf"),
  plt,
  width = standard_width_mm_double * 1.2,
  height = standard_height_mm * 1,
  units = "mm",
  dpi = standard_dpi_fig,
  scale = 2)
```
...
veh_qtl_net <- veh_global_net[
  rownames(veh_global_net) %in% snp_loc$SNP,
  colnames(veh_global_net) %in% cpg_veh_loc$CpG_ID,] %>%
...
# Take only QTLs
dex_qtl_net <- dex_global_net[
  rownames(dex_global_net) %in% snp_loc$SNP, 
  colnames(dex_global_net) %in% cpg_dex_loc$CpG_ID,]  %>%
  as.matrix() %>% as.data.frame()
...
plt <- ggplot(qtl_cnt, aes(x = treatment, y = n, color = treatment, fill = treatment)) +
  ...
  theme(legend.position = "bottom", # c(0.9, 0.9), 
        ...
        axis.text = element_text(size = standard_textsize, colour = "black")) + 
  labs(title = "", x = "", y = "Nr. of CpGs per SNP") +
  scale_color_manual(values = alpha(cbPalette, 0.75)) +
  scale_fill_manual(values = alpha(cbPalette, 0.25))

ggsave(
  paste0(plt_dir, "f04_gl_net_nr_cpgs_per_snp.pdf"),
  plt,
  width = standard_width_mm_double * 1,
  height = standard_height_mm * 2,
  units = "mm",
  dpi = standard_dpi_fig,
  scale = 1)
```
...
pdf(paste0(plt_dir, "f05_gl_net_circos_plt_", treatment, ".pdf"), compress = TRUE)
circos.clear()
# circos.initialize()
...
```{r out.width = "95%", include = T, eval = T}
(selected_meqtl <- intersect_cdm[1,]) # rs4881713 cg01572694 0.8213771     chr17  46657555     chr11  50327851

pdf(paste0(plt_dir, "f06_gl_net_box_plt_dev_veh.pdf"), compress = TRUE)
...
(selected_meqtl <- veh_cdm_unique[1, ]) # rs2324663 cg16400794 0.8511602    chr1 27709850    chr3 8649462 rs2324663-cg16400794

pdf(paste0(plt_dir, "f07_gl_net_box_plt_veh_only.pdf"), compress = TRUE)
ProcessGetBoxPlot(methyl_beta_veh_df, methyl_beta_dex_df, snps_mtrx, selected_meqtl, fdr.thr = 0.05)
dev.off()
```
...
pdf(paste0(plt_dir, "f08_gl_net_box_plt_dex_only.pdf"), compress = TRUE)
ProcessGetBoxPlot(methyl_beta_veh_df, methyl_beta_dex_df, snps_mtrx, selected_meqtl, fdr.thr = 0.05)
dev.off()
```